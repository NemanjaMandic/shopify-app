 ...
 def create_contest
   ...
   respond_to do |format|
     if @contest.save
       ...
       contest_results = ContestResult.new(candidates)
       
       # Save the winner
       @contest.update_attribute(:order_id, contest_results.results)
       
       format.html { redirect_to root_path, notice: "Contest Winner: <a href='#{orders_path(@contest.order)}'>#{@contest.order.email}</a>"}
